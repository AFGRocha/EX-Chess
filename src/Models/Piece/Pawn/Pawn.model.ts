...
export class Pawn extends Piece {
    ...
    getDiagonalPiecesAndDrawMove() {
        ...
        for (var moves in this.killablePieces) { 
            if(this.killablePieces[moves]) {
                ...
                const drawX = this.currentPosition.col - killablePiece.currentPosition.col
                const drawY = this.currentPosition.row - killablePiece.currentPosition.row
    ...
    enPasant() {
        // this.killablePieces.push(piecesInPlay.find(piece => JSON.stringify(piece.currentPosition) === JSON.stringify({col: this.currentPosition.col + 1, row: this.currentPosition.row}))!)
        // this.killablePieces.push(piecesInPlay.find(piece => JSON.stringify(piece.currentPosition) === JSON.stringify({col: this.currentPosition.col - 1, row: this.currentPosition.row}))!)
    }
}